...
const Dashboard = () => {
  ...
  const fetchSiteData = useCallback(async (siteUrl) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/gsc-data?site=${encodeURIComponent(siteUrl)}`,
        { credentials: 'include' }
      );
      if (response.ok) {
  ...
  useEffect(() => {
    const checkAuthAndFetchSites = async () => {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/api/gsc-sites`,
          { credentials: 'include' }
        );
        if (response.ok) {
  ...
  const handleSiteChange = (newSite) => {
    if (newSite !== selectedSite) setSelectedSite(newSite);
  };
    ...
    <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center space-y-4">
          <h1 className="text-4xl md:text-6xl font-bold gradient-text">GSC Traffic Forecast</h1>
          <p className="text-xl text-blue-200 max-w-3xl mx-auto">
            AI-powered Google Search Console analytics with predictive traffic forecasting
          </p>
          <div className="flex flex-wrap justify-center items-center gap-4 mt-6">
            <a href={`${import.meta.env.VITE_API_URL}/auth/google`}>
              <Button className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-300 glow-effect">
                <Activity className="w-5 h-5 mr-2" />
                {isLoggedIn ? 'Reconnect GSC' : 'Connect GSC'}
              </Button>
            </a>
            <Button onClick={handleDownload} variant="outline" className="border-white/20 text-white hover:bg-white/10 px-6 py-3 rounded-xl font-semibold">
            ...
            <div className="glass-effect p-4 rounded-xl flex flex-col md:flex-row items-center justify-center gap-4 border-green-500/50">
              ...
              {gscSites.length > 0 ? (
                <div className="flex items-center gap-4">
                  ...
                  <Button onClick={() => fetchSiteData(selectedSite)} className="bg-blue-600 text-white px-4 py-2 rounded-md">Predict</Button>
                </div>
              ) : (
        ...
        {/* Metrics */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <MetricCard title="Total Clicks" value={metrics.totalClicks.toLocaleString()} icon={<TrendingUp className="w-6 h-6" />} color="from-blue-500 to-cyan-500" loading={loading} />
          <MetricCard title="Avg CTR" value={metrics.avgCTR} icon={<Target className="w-6 h-6" />} color="from-purple-500 to-pink-500" loading={loading} />
          <MetricCard title="Avg Position" value={metrics.avgPosition} icon={<BarChart3 className="w-6 h-6" />} color="from-green-500 to-emerald-500" loading={loading} />
          <MetricCard title="Total Impressions" value={metrics.totalImpressions} icon={<Calendar className="w-6 h-6" />} color="from-orange-500 to-red-500" loading={loading} />
        </motion.div>

        {/* Forecast Controls */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="flex justify-center">
          <ForecastToggle forecastDays={forecastDays} onForecastChange={setForecastDays} />
        </motion.div>

        {/* Chart */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.6 }} className="chart-container rounded-3xl p-8">
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-white mb-2">Traffic Forecast Analysis</h2>
            <p className="text-blue-200">Historical data with {forecastDays}-day AI prediction</p>
          </div>
          <TrafficChart data={fullChartData} loading={loading} forecastDays={forecastDays} />
        </motion.div>

        {/* Backend Info */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.8 }} className="text-center text-blue-300 space-y-2">
          <p className="text-sm"><span className="font-semibold">Backend Endpoint:</span> {import.meta.env.VITE_API_URL}</p>
        </motion.div>
      </div>
    </div>
  );
